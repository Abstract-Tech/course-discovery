...
class CourseRunViewSetTests(SerializationMixin, ElasticsearchTestMixin, OAuth2Mixin, APITestCase):
    ...
    def test_editable_list_gives_drafts(self):
        # We delete self.course_run_2 and self.draft_course_run here so it doesn't show up in the response.
        # serialize_course_run returns an ordered list and we don't want to risk a flaky test
        # by including these course runs in there.
        ...
        # set key so it sorts later
        extra = CourseRunFactory(course__partner=self.partner, key=self.course_run.key + 'Z')

        response = self.client.get(reverse('api:v1:course_run-list') + '?editable=1')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['results'], self.serialize_course_run([draft, extra], many=True))
    ...
    def test_list_query_with_editable(self):
        """ Verify the endpoint returns a filtered list of courses """
        course_runs = CourseRunFactory.create_batch(3, title='Some random title', course__partner=self.partner)
        draft = CourseRunFactory(
            course__partner=self.partner, uuid=course_runs[0].uuid, key=course_runs[0].key, draft=True,
            title=course_runs[0].title, min_effort=-64549,
        )
        course_runs[0].min_effort = -10
        course_runs[0].draft_version = draft
        course_runs[0].save()
        # we are overwriting course_runs[0] with the draft because we are expecting only the draft to show up in the
        # results due to editable being True.
        official_version = course_runs[0]
        course_runs[0] = draft
        CourseRunFactory(title='non-matching name')
        query = 'title:Some random title'
        ...
        # Only the draft should be in the results. We are using min_effort as a proxy field to show it is
        # indeed the draft version and not the official_version
        matches = list(filter(lambda run: run['uuid'] == str(draft.uuid), actual_sorted))
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0]['min_effort'], draft.min_effort)
        self.assertNotEqual(matches[0]['min_effort'], official_version.min_effort)
